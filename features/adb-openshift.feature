Feature: Command output from various OpenShift related commands in ADB
  ...
  Scenario Outline: Boot and execute commands
    ...
    When I successfully run `sleep 20`
    ...
    And stdout from "bundle exec vagrant service-manager install-cli openshift --cli-version 1.3.0 --path #{ENV['VAGRANT_HOME']}/oc" should be evaluable in a shell
