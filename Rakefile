...
namespace :features do
  cucumber_config = YAML.load_file('./.config/cucumber.yml')
  cucumber_config.each_key do |key|
    next if key == 'default'
    Cucumber::Rake::Task.new(key.to_sym, "Run Cucumber features using profile '#{key}' (see cucumber.yml)'") do |t|
      t.profile = key
    end
  end
